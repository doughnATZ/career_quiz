...
function showResult() {
    ...
    questions.forEach((q, index) => {
        scores[q.cluster] = (scores[q.cluster] || 0) + (answers[index] || 0);
    });

    const maxScore = Math.max(...Object.values(scores));
    const topClusters = Object.keys(scores).filter(cluster => scores[cluster] === maxScore);

    topCluster.textContent = topClusters.join(", ");
