...
const eval1Group = document.getElementById("eval1-group");
const eval2Group = document.getElementById("eval2-group");
const eval3Group = document.getElementById("eval3-group");
const eval4Group = document.getElementById("eval4-group");
...
submitBtn.addEventListener("click", () => {
    ...
    // Optionally, save data to Google Sheets via Google Apps Script
});
...
function showQuestion() {
    ...
    q.programs.forEach((prog, index) => {}); // placeholder
...
function calculateCluster() {
    const clusters = ["Natural Resources & Agriculture","Human Services","Industrial & Engineering Technology","Health Services","Arts & Communication","Business, Management & Technology"];
    // tally logic
    clusterResult.textContent = clusters[Math.floor(Math.random()*clusters.length)]; // placeholder
}

// SHOW TOP COURSES
function showTopCourses() {
    courseResults.innerHTML = "<ol><li>Course 1</li><li>Course 2</li><li>Course 3</li></ol>"; // placeholder
}

// SHOW EVALUATION
function showEvaluation() {
    for (let i=1;i<=5;i++){
        const group = document.getElementById("eval"+i+"-group");
        if(i===1 || i===2){
            group.innerHTML = "";
            for(let j=1;j<=5;j++){
                const btn = document.createElement("button");
